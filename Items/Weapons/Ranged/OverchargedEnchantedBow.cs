...
namespace CelestialInfernalMod.Items.Weapons.Ranged
{
	public class OverchargedEnchantedBow : ModItem
	{
		public override void SetStaticDefaults()
		{
			...
			Tooltip.SetDefault("Wooden arrows are converted to enchanted beams");
		}
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			for (int i = 0; i < numberProjectiles; i++)
			{
				...
				int arrow = Projectile.NewProjectile(position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, type, damage, knockBack, player.whoAmI);
				Main.projectile[arrow].Celestial().forceRanged = true;
