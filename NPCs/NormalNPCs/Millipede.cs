...
namespace CelestialInfernalMod.NPCs.NormalNPCs
{
	internal class MillipedeHead : Millipede
	{
        ...
        {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.WormTooth, Main.rand.Next(1, 3));
        }
	}
	...
	public abstract class Millipede : Worm
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Millipede");
		}

		public override void Init() {
			minLength = 10;
			maxLength = 20;
			tailType = NPCType<MillipedeTail>();
			bodyType = NPCType<MillipedeBody>();
			headType = NPCType<MillipedeHead>();
			speed = 5f;
			turnSpeed = 0.045f;
		}
	}
	...
	public abstract class Worm : ModNPC
	{
		...
		public override void AI() {
			...
			if (Main.netMode != 1) {
				if (!tail && npc.ai[0] == 0f) {
					if (head) {
						...
						npc.ai[0] = (float)NPC.NewNPC((int)(npc.position.X + (float)(npc.width / 2)), (int)((float)npc.height + 20), bodyType, npc.whoAmI);
					}
					else if (npc.ai[2] > 0f) {
			...
			int num183 = (int)((npc.position.Y) / 16f) + 2;
			if (num180 < 0) {
				num180 = 0;
			}
			...
			if (npc.ai[1] > 0f && npc.ai[1] < (float)Main.npc.Length) {
				try {
					...
					num192 = Main.npc[(int)npc.ai[1]].position.Y + (float)(Main.npc[(int)npc.ai[1]].height) - vector18.Y;
				}
				catch {
				}
