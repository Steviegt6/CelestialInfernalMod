...
namespace CelestialInfernalMod.NPCs.Ghosts
{
    public class LeadGhost : ModNPC
    {
        ...
        {
            Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.LeadOre, Main.rand.Next(5, 13));
            if (Main.rand.Next(10) < 1)
            {
                Item.NewItem(npc.getRect(), ItemID.LeadBroadsword, Main.rand.Next(1, 2));
            }
        }
    }
}
