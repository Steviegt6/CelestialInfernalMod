...
namespace CelestialInfernalMod.NPCs.Ghosts
{
    public class GoldGhost : ModNPC
    {
        ...
        {
            Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.GoldOre, Main.rand.Next(5, 13));
            if (Main.rand.Next(10) < 1)
            {
                Item.NewItem(npc.getRect(), ItemID.GoldBroadsword, Main.rand.Next(1, 2));
            }
        }
    }
}
